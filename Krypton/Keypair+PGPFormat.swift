...
extension KeyPair {
    ...
    */
    ...
    func exportAsciiArmoredPGPPublicKey(for identities:[String], created:Date = Date(), hashAlgorithm:PGPFormat.Signature.HashAlgorithm = .sha512) throws -> AsciiArmorMessage {
        return try createPGPPublicKeyMessage(for: identities, created: created, hashAlgorithm: hashAlgorithm).armoredMessage(blockType: .publicKey, comment: Properties.defaultPGPMessageComment)
    }
    ...
    */
    func createAsciiArmoredBinaryDocumentSignature(for binaryData:Data, using hashAlgorithm:PGPFormat.Signature.HashAlgorithm = .sha512, keyID:Data, comment:String) throws -> AsciiArmorMessage {
        ...
        return try signedBinary.toMessage().armoredMessage(blockType: .signature, comment: comment)
    }
    
    /**
        Create a PGP Signature over a Git Commit
     */
    func signGitCommit(with commitInfo:CommitInfo, keyID:Data, comment:String) throws -> AsciiArmorMessage {
        return try self.createAsciiArmoredBinaryDocumentSignature(for: commitInfo.data, keyID: keyID, comment: comment)
    }
    
    /**
        Create a PGP Signature over a Git Tag
     */
    func signGitTag(with tagInfo:TagInfo, keyID:Data, comment:String) throws -> AsciiArmorMessage {
        return try self.createAsciiArmoredBinaryDocumentSignature(for: tagInfo.data, keyID: keyID, comment: comment)
    }
}
